...
node.set_ip_and_port(sys.argv[1], sys.argv[2]) # need to set for node connections
...
for i in range(0, num_connections):
    ...
    #node.connect_to_node(sys.argv[ip_index], sys.argv[port_index])

# boot the node with the given connections
node.boot(sys.argv[1], sys.argv[2])
